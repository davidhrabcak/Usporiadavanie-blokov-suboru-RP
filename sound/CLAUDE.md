...
**New data point (2026-07-09, real-world VBR file `test7_shorter.mp3`)**: unlike `sample-3s.mp3`, `mainDataBegin` here is *not* saturated - across all 859 true frames it spans the full 0-511 range (only 2 frames are 0, only 44% exceed 480). This is exactly the discriminating signal item 6/7 below need: a correct (subtracting) reservoir model should reject a meaningful fraction of false-positive successor edges in VBR content, whereas it would have been nearly useless on `sample-3s.mp3`. See the TODO "Implement real `part2_3_length`-based reservoir consumption" - this is now the highest-leverage remaining fix for VBR feasibility, not the CBR-oriented ideas below.
...
**Status: root-caused, fix not yet applied - see TODO below.**
...
- [ ] **Fix `computeChunkMeta`'s validity gate for VBR (Known Limitations item 6).**
  In `chunk_meta.cpp:206`, change `if (bestOff >= 0 && bestCount >= 1)` to `if (bestOff >= 0)`. A chunk should be considered valid whenever *any* candidate offset decodes to a profile-matching header with usable tail info (`tailQuality(bestTail, bestTailLen) >= 1`, i.e. Case 1 or Case 2 recoverable) - it should not require a *complete* frame to fit, since that's routine to fail for larger VBR frames relative to `CHUNK_SIZE`. Concretely: keep picking `bestOff`/`bestTail`/`bestTailLen` exactly as today (the `(tailQuality, count)` ranking doesn't need to change), just stop discarding a winning candidate whose `count` happens to be 0. After changing, rerun the Phase-1 pre-shuffle check against `test7.mp3`/`test7_short.mp3`/`test7_shorter.mp3` (currently 17 breaks on the shorter clip; expect 0, matching e.g. `sample-3s.mp3`/`CBR.mp3` today) before touching anything else - this is a prerequisite for item 7's numbers to mean anything, since right now ~9/320 chunks are graph islands regardless of what Case-1/Case-2 logic does.

- [ ] **Implement real `part2_3_length`-based reservoir consumption (supersedes the "model consumption" half of Known Limitations item 3).**
  The previous attempt (subtracting `mainDataBegin` as a stand-in for consumption) was wrong and was reverted - see item 3. The correct quantity is `part2_3_length`, a 12-bit field per granule per channel inside the Layer III side info (2 granules × up to 2 channels for MPEG1; 1 granule × up to 2 channels for MPEG2/2.5), immediately following `main_data_begin`/`private_bits`/`scfsi`. Sum of `part2_3_length` (bits) across all granules/channels in a frame, rounded up to bytes, is the actual main-data bytes that frame consumes from the reservoir - this should be *subtracted* from `state.reservoirLevel` in `advanceReservoirFrame` (main.cpp) in addition to the existing accumulate step, turning the current one-directional "has enough ever accumulated" check into a real two-sided consumption model. Needs: (1) a bit-level reader for the side-info block (byte-aligned start, non-byte-aligned fields inside - `main_data_begin` is 9 bits, `part2_3_length` is 12 bits, etc., per ISO/IEC 11172-3), (2) validate against a file with a known-correct order before trusting it, the same way the `mainDataBegin` regression was caught on `sample-3s.mp3` - use `test7_shorter.mp3` too, since its `mainDataBegin` is *not* saturated (0-511, only 44% > 480; see item 3's 2026-07-09 update), so it should give a much more discriminating signal there than on `sample-3s.mp3`. This is expected to be the highest-leverage fix for item 7's branching factor, since it adds a genuinely independent constraint per edge instead of relying only on "is there a decodable header at this byte".
