...
- [x] **Fix `computeChunkMeta`'s validity gate for VBR (Known Limitations item 6).**
  Implemented in commit `c83ffda` (this checkbox and item 6's status line were just never updated to match at the time - see item 6). `chunk_meta.cpp:206` reads `if (bestOff >= 0)`. Verified live: `test7.mp3`/`test7_short.mp3`/`test7_shorter.mp3` all now pass the Phase-1 pre-shuffle check with 0 breaks.
